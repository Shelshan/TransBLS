...
import math
from BLS import BLStrain, BLS_AddEnhanceNodes, BLS_AddFeatureEnhanceNodes, bls_train_input, bls_train_inputenhance
...
# 未微调
#image_size = 224
# base_model = vit_or.vit_t16(image_size=image_size,activation='sigmoid',pretrained=False,include_top=False,pretrained_top=False)
#base_model = vit_or.vit_s16(image_size=image_size,activation='sigmoid',pretrained=False,include_top=False,pretrained_top=False)
#base_model = vit_or.vit_b16(image_size=image_size,activation='sigmoid',pretrained=False,include_top=False,pretrained_top=False)
# base_model = vit_or.vit_s32(image_size=image_size,activation='sigmoid', pretrained=False,include_top=False,pretrained_top=False)
# base_model = vit.vit_b32(image_size=image_size,activation='sigmoid',pretrained=True,include_top=False,pretrained_top=False)
#base_model = vit_or.vit_h14(image_size=image_size,activation='sigmoid',pretrained=False,include_top=False,pretrained_top=False)
...
if __name__ == '__main__':
    ...
    # 获取数据
    # SCUT-FBP_dataset
    # filePath_train = r'F:\\XieXS\\datasets\\SCUT-FBP\\train.txt'
    # filePath_test = r'F:\\XieXS\\datasets\\SCUT-FBP\\test.txt'
    ...
    # np.save('./data/SCUT-FBP5500_T32/traindata.npy', traindata)
    # np.save('./data/SCUT-FBP5500_T32/trainlabel.npy', trainlabel)
    testdata, testlabel = getDatanew(filePath_test, colorType=cv2.IMREAD_COLOR,needGenImg=False)
    # np.save('./data/SCUT-FBP5500_T32/testdata.npy', testdata)
    # np.save('./data/SCUT-FBP5500_T32/testlabel.npy', testlabel)
    t3=time.time()
    datatime = t3-t1
